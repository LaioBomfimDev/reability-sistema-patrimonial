...
export const useAssets = () => {
  ...
  const updateAsset = useCallback(async (id, assetData) => {
    ...
    try {
      ...
      toast.success(TOAST_MESSAGES.SUCCESS.ASSET_UPDATED);
      return data;
    } catch (err) {
  ...
  // Move asset (transfer)
  const moveAsset = useCallback(async (assetId, movementData) => {
    setLoading(true);
    setError(null);
  ...
  const returnValue = useMemo(() => ({
    ...
    fetchAssets, createAsset, updateAsset, deleteAsset, moveAsset, getUniqueLocations, clearCache
  ]);
